...
class PurpleAir(object):
    ...
    def dataline_is_minute_data(dataline):
        ...
        # should be 21 columns in a data line
    ...
    def find_purpleairs():
        ...
        usb_module_path = '/sys/bus/usb/drivers/cp210x/' # Ruggeduino
        #usb_module_path = '/sys/bus/usb/drivers/ch341/' # Arduino Nano Every
        ...
        if paths:
            print("Possible Rugged Air devices at paths:")
            for path in paths:
                f = open(path.split('tty')[0] + 'interface', 'r')
                if 'Rugged' in f.readline():
                    rugged_path = path
                    print("\t" + rugged_path)
        devices = [os.path.basename(rugged_path)]
