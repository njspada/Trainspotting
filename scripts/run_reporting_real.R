...
save_da <- function(da, day) {
  ...
  # print(nrow(da$da))
	...
	post_df(da$train_images, train_images_file$fpath, train_images_file$fname, "train_imagesi_simple")
	
	post_image <- function(filename) {
		body = list(type="image", file=upload_file(filename), device_id=device_id, filename=filename)
		r <- POST(post_url, body=body, encode="multipart")
    # print(content(r,"text"))
	}
