...
drive_auth('njspada@ucdavis.edu')

out.dir <- "/usr/local/output/"
...
report_daily <- function(day) {
  ...
  if (nrow(total) > 0) {
    ...
    drive_upload(fpath, paste0("/Trainspotting/FieldLogs/", fname))
  }
}
...
run_service <- function() {
  ...
  # Upload the daily file at 5 minutes past midnight each day
